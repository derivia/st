...
void xloadcols(void)
{
  ...
  for (i = 0; i < dc.collen; i++)
    if (!xloadcolor(i, NULL, &dc.col[i])) {
      if (colorname[i]) die("could not allocate color '%s'\n", colorname[i]);
...
void xloadfonts(const char *fontstr, double fontsize)
{
  ...
  if (fontstr[0] == '-') pattern = XftXlfdParse(fontstr, False, False);
  else pattern = FcNameParse((const FcChar8 *)fontstr);
...
void xinit(int cols, int rows)
{
  ...
  if (!(opt_embed && (parent = strtol(opt_embed, NULL, 0)))) parent = root;
  xw.win = XCreateWindow(xw.dpy, root, xw.l, xw.t, win.w, win.h, 0,
                         ...
                         &xw.attrs);
  if (parent != root) XReparentWindow(xw.dpy, xw.win, parent, xw.l, xw.t);
